...
class OllamaManager {
  ...
  async installOllama(progressCallback) {
    try {
      ...
      if (process.platform === 'win32') {
        // Windows: Show installer UI, let user click Install, track progress
        progressCallback && progressCallback(5, 'Launching AI Brain installer...');
        progressCallback && progressCallback(10, 'Please click Install in the installer window');
        
        await new Promise((resolve, reject) => {
          // Launch installer with UI visible (no silent flags)
          // Try multiple possible locations for OllamaSetup.exe
          const possiblePaths = [
            path.join(__dirname, '../../resources/binaries/OllamaSetup.exe'),
            path.join(process.cwd(), 'resources/binaries/OllamaSetup.exe'),
            path.join(app.getAppPath(), 'resources/binaries/OllamaSetup.exe'),
            path.join(__dirname, '../../OllamaSetup.exe'),
            path.join(process.cwd(), 'OllamaSetup.exe')
          ];
          
          let installerPath = null;
          for (const p of possiblePaths) {
            console.log(`Checking for installer at: ${p}`);
            if (fs.existsSync(p)) {
              installerPath = p;
              console.log(`Found installer at: ${p}`);
              break;
            }
          }
          ...
          if (!installerPath) {
            ...
            reject(new Error(`Installer not found. Searched in:\n  - ${searchedPaths}\n\nPlease place OllamaSetup.exe in resources/binaries/ folder.`));
            return;
          }
          
          console.log(`Launching installer from: ${installerPath}`);
          ...
          installer.unref(); // Allow installer to run independently
          
          console.log('Installer launched, waiting for user to click Install...');
          
          let installProgress = 10;
          let checkCount = 0;
          
          // Monitor installation by checking if Ollama gets installed
          const checkInterval = setInterval(async () => {
            checkCount++;
            
            // Check if Ollama is now installed
            const installed = this.isOllamaInstalled();
            console.log(`Check ${checkCount}: Ollama installed = ${installed}`);
            ...
            // Update progress (10% to 90% over ~2 minutes)
            if (installProgress < 90) {
              installProgress += 2;
              
              // User-friendly messages
              if (installProgress < 30) {
                progressCallback && progressCallback(installProgress, 'Waiting for installation to start...');
              } else if (installProgress < 60) {
                progressCallback && progressCallback(installProgress, 'Installing AI Brain components...');
              } else {
                progressCallback && progressCallback(installProgress, 'Finalizing installation...');
              }
            }
            
            // Timeout after 5 minutes (150 checks * 2s)
            if (checkCount > 150) {
              clearInterval(checkInterval);
              reject(new Error('Installation timed out. Please try again.'));
            }
          }, 2000); // Check every 2 seconds
          
          installer.on('error', (error) => {
            clearInterval(checkInterval);
            // Don't reject on error - installer might still work
            console.error('Installer spawn error (may be OK):', error);
          });
        });
