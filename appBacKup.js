...
//     if (retry) {
//       if (data == "Docker is already installed.") {
...
//         logToConsole("Docker is already installed.", "success");
//       }
...
//         if (detectSoftwareResult === "Docker is already installed.") {
...
async function runDocker() {
  if (window.electronAPI && window.electronAPI.openDockerDesktop) {
    ...
    let detectDockerRunningInterval = setInterval(async () => {
      const detectDockerRunningResult = await detectDockerRunning();

      // If n8n is installed in Docker, stop the interval
      if (detectDockerRunningResult.message === "Docker is installed and running.") {
        clearInterval(detectDockerRunningInterval); // Stop the interval when condition is met
        document.querySelector("#step1").classList.remove("active");
        document.querySelector("#step2").classList.add("active");
        document.querySelector("#step3").classList.remove("active");
        document.querySelector("#step4").classList.remove("active");
